...
std::vector<PlanetData> loadPlanetData(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string val;
        // read x
        std::getline(ss, val, ','); pd.x = std::stod(val);
        // read y
        std::getline(ss, val, ','); pd.y = std::stod(val);
        // read z
        std::getline(ss, val, ','); pd.z = std::stod(val);
        // read vx
        std::getline(ss, val, ','); pd.vx = std::stod(val);
        // read vy
        std::getline(ss, val, ','); pd.vy = std::stod(val);
        // read vz
        std::getline(ss, val, ','); pd.vz = std::stod(val);
        // read M
        std::getline(ss, val, ','); pd.M = std::stod(val);
